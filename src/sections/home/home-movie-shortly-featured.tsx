...
export default function MoviesShortlyFeatured({ list }: Props) {
  ...
  return (
    ...
    >
      ...
      <Carousel ref={carousel.carouselRef} {...carousel.carouselSettings} swipeToSlide>
        {list.map((item) => (
          <MoviesShortlyItem key={item.id} item={item} />
        ))}
      </Carousel>
    </Box>
  );
...
function MoviesShortlyItem({ item }: MovieItemProps) {
  const settings = useSettingsContext();

  const { classificacaoIndicativa, titulo, urlCapa, dataEstreia } = item;
    ...
    >
      ...
      <Typography
        sx={{
          ...
          fontWeight: 'bold',
          textAlign: 'center',
          textShadow: '2px 2px 8px rgba(0, 0, 0, 0.70)',
          color: '#ffffff',
        }}
      >
        {dataEstreia}
